...
import com.recaudo.api.domain.model.dto.response.UserPermissionProjection;
...
public interface ModuleActionRepository extends JpaRepository<ModuleActionEntity, Long> {
    ...
    @Query(value = """
                ...
                GROUP BY mo.id, mo.name, ac.id, ac.name, up.id, rp.id, up.allow, rp.allow
            """, nativeQuery = true)
    List<UserPermissionProjection> findPermissionsByUserId(@Param("userId") Long userId);
